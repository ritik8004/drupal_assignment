...
class SkuImagesManager {
   ...
   */
  public function getGallery(SKUInterface $sku, $check_parent = FALSE) {
    /** @var \Drupal\acq_sku\Entity\SKU $sku */
    // @TODO: Think of static cache.
    $media = $sku->getMedia();
    ...
    if (!empty($media)) {
      foreach ($media as $media_item) {
        ...
        if ($media_item['media_type'] == 'image') {
          ...
          $gallery['images'][$url] = $url;
        }
        elseif ($media_item['media_type'] == 'video') {
          $gallery['videos'][$media_item['video_url']] = $media_item['video_url'];
        }
      }
    }
