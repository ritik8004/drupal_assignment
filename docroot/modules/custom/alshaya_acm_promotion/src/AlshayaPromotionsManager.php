...
class AlshayaPromotionsManager {
  ...
  /**
   * Get promotions to show for particular sku in cart for api.
   *
   * @param string $sku
   *   SKU code.
   * @param string $applied_coupon
   ...
   *   Promotions array.
   */
  public function getPromotionsForSkuInCart(string $sku, $applied_coupon = '') {
    $static = &drupal_static('getPromotionsForSkuInCart', []);
    ...
    foreach ($line_item_promotions as $promotion_id => $promotion) {
      ...
      if ($promotion['promo_type'] == SkuManager::FREE_GIFT_SUB_TYPE_ONE_SKU) {
        ...
        $message_arguments['product_text'] = $promotion['text'];
        ...
        if ($free_sku_entity->bundle() == 'simple') {
          ...
          $message_arguments['sku_entity_id'] = $free_sku_entity->id();
          $message_arguments['product_text'] = $free_sku_entity->label();
        }
        $free_gift_promos[$promotion_id]['link'] = $message_arguments;
      }
    }
