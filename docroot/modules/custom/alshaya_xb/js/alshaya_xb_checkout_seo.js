...
(function (Drupal, dataLayer) {
  ...
  window.glegem("OnCheckoutStepLoaded", function seo(data) {
    if (data !== undefined) {
      if (data.StepId === data.Steps.LOADED) {
        dataLayer.push(Drupal.getGlobaleGaCheckoutData(data, 2));
      }
      else if (data.StepId === data.Steps.CONFIRMATION) {
        dataLayer.push(Drupal.getGlobaleGaCheckoutData(data, 4));
      }
    }
  });
