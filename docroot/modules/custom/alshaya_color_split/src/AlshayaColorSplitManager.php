...
class AlshayaColorSplitManager {
  ...
  public function alterGroupAttributeFormItem(array &$configurations, array $options, $grouping_attribute) {
    $pdp_swatch_attribute = $this->skuManager->getPdpSwatchAttributes();
    $pdp_swatch_attribute = reset($pdp_swatch_attribute);
    $sku_configurable_options_color = [];
    if ($pdp_swatch_attribute === $grouping_attribute) {
        ...
        if (!empty($swatch)) {
          switch ($swatch['type']) {
            ...
            case SwatchesHelper::SWATCH_TYPE_VISUAL_COLOR:
              ...
              $configurations['#attached']['drupalSettings']['sku_configurable_options_color'] = $sku_configurable_options_color;
              break;
