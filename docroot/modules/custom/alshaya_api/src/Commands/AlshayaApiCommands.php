...
class AlshayaApiCommands extends DrushCommands {
   ...
   *
   * @command alshaya_api:sanity-check-product-aliases
   * @options fix
   *   Whether to fix the translations or not.
   *
   * @aliases aascpa
   *
   * @usage alshaya-api:sanity-check ar
   *   Checks and mentions the count and nids of untranslated AR nodes.
   * @usage alshaya-api:sanity-check en --fix
   *   Checks and mentions the count and nids of untranslated EN nodes and
   *   generates EN aliases for them.
   */
  public function sanityCheckAndFixProductAlias(string $langcode = 'en', array $options = ['fix' => FALSE]) {
    ...
    if ($options['fix']) {
      ...
      $this->output()->writeln('Re-check if process was successful by running the same command.');
    }
    else {
      if (!empty($nids)) {
