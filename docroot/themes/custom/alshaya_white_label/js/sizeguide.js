    ...
    attach: function (context, settings) {
      ...
      if ($('#size-select').length == 0) {
        $('.form-item-configurables-size select').after("<ul id='size-select' />")
          ...
          var bh = $('.form-item-configurables-size select option:selected').text();
        $('#size-select').before('<div><span class="size-label">Size : </span><span class="size-value">'+bh+'</span></div>');
      }
