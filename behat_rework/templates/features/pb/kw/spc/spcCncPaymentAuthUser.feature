...
Feature: SPC Checkout using Click & Collect store for Authenticated user
  ...
  Scenario: As a authenticated user, I should be able to checkout using click and collect with credit card
    ...
    And I wait for the page to load
    And I click jQuery "#spc-checkout .spc-main .spc-content .spc-checkout-delivery-methods #delivery-method-cnc" element on page
    And I wait for AJAX to finish
...
#  And the element "#block-content .spc-main .spc-sidebar .spc-order-summary-block .product-item .spc-product-attributes" should exist
  ...
  @cc @cnc @knet
  Scenario: As a authenticated user, I should be able to checkout using click and collect with knet
    When I select a product in stock on ".views-element-container.block.block-views.block-views-blockalshaya-product-list-block-1"
    And I wait 10 seconds
    And I wait for the page to load
    When I press "{add_to_cart_link}"
