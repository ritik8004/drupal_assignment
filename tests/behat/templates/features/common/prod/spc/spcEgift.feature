@javascript @guest @e-gift @bbwsaprod
Feature: SPC Checkout Egift feature for Guest user

  Background:
    Given I am on "{spc_egift_page}"
    And I wait for element ".c-content .paragraph-image_title_subtitle_link"
  ...
  Scenario: As a Guest user, I should be able to use E-gift feature on the arabic page
    ...
    And I wait for element ".checkout-login-wrapper"
    Then I am logged in as an authenticated user "{spc_auth_user_email}" with password "{spc_auth_user_password}"
    And I wait for element "#delivery-method-home_delivery"
    And I should see an ".redeem-egift-card" element
    Then I select the Checkout payment method
    And I wait for element "input#payment-method-checkout_com_upapi[checked]"
