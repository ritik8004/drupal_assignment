...
foreach ($conductors as $key => $value) {
  ...
  if (is_object($queue_count) && isset($queue_count->total) && $queue_count->total <= $limit) {
    update_queue_status_call('resuming', $value['site_id'], [$base_env, $brand, $country]);
    if ($mode === 'report') {
      echo 'Queue resumed for ' . $value['site_id'] . ' : ' . $country_brand . ' : ' . $base_env . '. Queue size: ' . $queue_count->total;
      echo PHP_EOL;
    }
  }
}
...
function update_queue_status_call($site_id, $args = []) {
  global $mode;
  if ($mode !== 'report') {
    echo '=> Resuming queue for site_id ==> ' . $site_id . ' ==> (' . json_encode($args) .')';
    echo PHP_EOL;
  }
  $data = update_queue_status($site_id, FALSE);
  if ($mode !== 'report') {
