...
//        }
    ...
    public function theUrlShouldBeCorrect($arg1) {
        ...
        if ($current_url == NULL) {
            ...
            $expected_url = $this->base_url . "/search?keywords=$arg1";
        }
...
//        }
    ...
    /**
     * @Then /^it should be added to the basket and the cart should reflect correct total$/
     */
    public function itShouldBeAddedToTheBasketAndTheCartShouldReflectCorrectTotal() {

        $page = $this->getSession()->getPage();
        //    $product_name = $page->find('css','.content__title_wrapper h1')->getText();
        $page->hasContent($this->product . ' has been added to your cart. and KWD');
        $page->hasLink('view cart');
        $original_price = (float) $page->findById('block-alshaya-white-label-content')
            ->find('css', '.content__title_wrapper .price .price-amount')
            ->getText();
        $original_price = $original_price * $this->quantity;
        $this->iWaitForThePageToLoad();
        $cart_price = (float) $page->findById('mini-cart-wrapper')
            ->find('css', '.acq-mini-cart a.cart-link-total .price-amount')
            ->getText();

        if ($original_price !== $cart_price) {
            throw new Exception('Value displayed in the basket is incorrect');
        }
    ...
    /**
     * @Then /^it should be added to the basket and the cart should reflect correct total for Arabic site$/
     */
    public function itShouldBeAddedToTheBasketAndTheCartShouldReflectCorrectTotalForArabicSite() {

        $page = $this->getSession()->getPage();
        $product_name = $page->find('css', '.content__title_wrapper h1')->getText();
        $page->hasContent($product_name . '  تم إضافة القطعة/القطع إلى حقيبتك      and د٠ك٠');
        $page->hasLink('عرض سلة التسوق');
        $original_price = (float) $page->find('css', '.content__title_wrapper .special--price .price-amount')
            ->getText();
        $original_price = $original_price * $this->quantity;
        $cart_price = (float) $page->findById('block-cartminiblock')
            ->findById('mini-cart-wrapper')
            ->find('css', '.acq-mini-cart a.cart-link-total .price-amount')
            ->getText();
    ...
    public function iShouldSeeAMessageForTheProductBeingAddedToCartInArabic($arg1) {
        $page = $this->getSession()->getPage();
        $actual = $page->find('css', '.notification div.col-2')->getText();
...
//        }
    ...
    public function iClickAPointerOnTheMapOnArabicSite() {
        $this->getSession()
            ->getPage()
            ->find('css', '#map-canvas-59d71d2db6ceb > div.geolocation-google-map.geolocation-processed > div > div > div:nth-child(1) > div:nth-child(4) > div:nth-child(3) > div > img')
            ->click();
    }
    ...
    public function iShouldSeeTitleAddressOpeningHoursAndGetDirectionsLinkOnThePopup() {
        ...
        $address = $page->has('css', 'div.views-field.views-field-field-store-address > div.field-content');
        if (!$address) {
    ...
    public function iShouldSeeResultsSortedInDescendingOrder() {
        ...
        if (!$this->is_array_ordered($actual_values, ORDER_DSC)) {
            throw new Exception('Search results list is not sorted in descending order');
        }
    }
...
//        }
    ...
    public function iShouldSeeAllOrdersListedOnOrdersTab($arg1) {
        ...
        if (strtolower($status_name) !== strtolower($arg1)) {
    ...
    /**
     * @Then /^the video should be downloaded in a new tab$/
     */
    public function theVideoShouldBeDownloadedInANewTab() {
        $windowNames = $this->getSession()->getWindowNames();
        if (count($windowNames) > 1) {
            $this->getSession()->switchToWindow($windowNames[1]);
        }
    ...
    public function itShouldDisplaySizeAndQuantity() {
        $page = $this->getSession()->getPage();
        $eng_size = $page->find('css','#configurable_ajax > div.form-item.js-form-type-select.form-type-select.form-item-configurables-size > div.select2Option > h4.list-title')
            ->find('named',array('content','Size : '));
        $arb_size = $page->find('css','#configurable_ajax > div.form-item.js-form-type-select.form-type-select.form-item-configurables-size > div.select2Option > h4')
    ...
    {
        $page = $this->getSession()->getPage();
        $actual_subtotal = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(1) > td:nth-child(2) > div > div > span > div > span.price-amount')->getText();
        $expected_discount = $actual_subtotal - $discount;
        $actual_discount = $actual_subtotal + (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(2) > td:nth-child(2) > div > div.price > span > div > span.price-amount')->getText();
    ...
    {
        $page = $this->getSession()->getPage();
        $eng_size = $page->find('css','#configurable_ajax > div.form-item-configurables-article-castor-id > div.select2Option > h4.list-title')
            ->find('named',array('content','Color : '));
        $arb_size = $page->find('css','#configurable_ajax > div.form-item-configurables-article-castor-id > div.select2Option > h4.list-title')
    ...
    /**
     * @Then /^I should get "([^"]*)" percent discount if the cart value is greater than "([^"]*)" KWD$/
     */
    ...
    /**
     * @Then /^I should get a discount of "([^"]*)" KWD on the order total$/
     */
    public function iShouldGetADiscountOfKWDOnTheOrderTotal($discount_price)
    {
        $page = $this->getSession()->getPage();
        $sub_total = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(1) > td:nth-child(2) > div > div > span > div > span.price-amount')->getText();
        $actual_discount = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(2) > td:nth-child(2) > div > div.price > span > div > span.price-amount')->getText();
        $expected_discount = $sub_total - $discount_price;
        if($actual_discount !== $expected_discount){
            throw new Exception('Discount of '. $discount_price. ' did not get applied');
        }
    }

    /**
     * @Then /^I should get a discount of "([^"]*)" KWD on the order total when quantity is atleast "([^"]*)"$/
     */
    public function iShouldGetADiscountOfKWDOnTheOrderTotalWhenQuantityIsAtleast($discount_price, $quantity)
    {
        $page = $this->getSession()->getPage();
        $all_quantity = $page->findAll('css','.selection');
        $total_quantity = 0;
        foreach ($all_quantity as $per_quantity){
            $total_quantity = $total_quantity + $per_quantity->find('css','.select2-selection > span')->getText();
        }
        $actual_discount = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(2) > td:nth-child(2) > div > div.price > span > div > span.price-amount')->getText();
        if($total_quantity >= $quantity){
            $sub_total = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(1) > td:nth-child(2) > div > div > span > div > span.price-amount')->getText();
            $actual_total = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(3) > td:nth-child(2) > div > div > span > div > span.price-amount')->getText();
            $expected_total = $sub_total - $discount_price;
            if($actual_total !== $expected_total){
                throw new Exception('Discount of '. $discount_price . ' KWD did not get applied');
            }
        }
        else if ($actual_discount == true){
            throw new Exception('Discount is being applied even when the minimum quantity criteria is not being met');
        }
    }

    /**
     * @Then /^I should get a discount of "([^"]*)" percent on the order total$/
     */
    public function iShouldGetADiscountOfPercentOnTheOrderTotal($discount_percent)
    {
        $page = $this->getSession()->getPage();
        $sub_total = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(1) > td:nth-child(2) > div > div > span > div > span.price-amount')->getText();
        $actual_discount = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(2) > td:nth-child(2) > div > div.price > span > div > span.price-amount')->getText();
        $actual_total = (float)$page->find('css','#table-cart-totals > table > tbody > tr:nth-child(3) > td:nth-child(2) > div > div > span > div > span.price-amount')->getText();
        $discounted_price = $sub_total * $discount_percent;
