...
/**
 * @file
 * This file aims to rotate custom logs on Alshaya servers.
 *
 * Scheduled tasks on ACE are configured to log on custom log files which
 * the pattern is "alshaya-<something>.log". This script will identifies
 * the files matching the pattern and which are bigger than ~10Mo.
 *
...
foreach (array_diff(scandir($directory), ['..', '.']) as $item) {
  ...
  if (filesize($directory . $item) < 10_000_000) {
