...
stage_res=$(curl -sk "https://www.alshaya.acsitefactory.com/api/v1/stage" -u ${username}:${api_key})
...
for env in $envs ; do
  ...
  vcs_res=$(curl -sk "https://www.$env-alshaya.acsitefactory.com/api/v1/vcs?type=sites&stack_id=4" -u ${username}:${api_key})
  curr_branch=$(php -r '$json = '"'$vcs_res'"'; echo json_decode($json)->current;')
  echo "$env: $curr_branch"
done
